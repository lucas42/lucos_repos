...
func init() {
	Register(Convention{
		...
		Description: "No Dockerfile built by a non-test-profile service in docker-compose.yml has a Dependabot-blind image reference: a COPY --from=<external-image> without a matching named FROM stage, a FROM using ARG variable substitution, or an additional_contexts entry using docker-image://",
